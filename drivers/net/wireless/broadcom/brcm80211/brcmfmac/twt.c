 ...
 */
static const char *brcmf_twt_session_state_str[BRCMF_TWT_SESS_STATE_MAX] = {
	"Unspecified",
	"Setup in progress",
	"Setup complete",
 ...
 */
static inline u8 brcmf_twt_wake_dur_to_min_twt(u32 wake_dur, u8 min_twt_unit)
{
	u8 min_twt;
 ...
 */
static inline u32 brcmf_twt_min_twt_to_wake_dur(u8 min_twt, u8 min_twt_unit)
{
	u32 wake_dur;
 ...
 */
static inline void brcmf_twt_u32_to_float(u32 wake_int, u8 *exponent,
					  u16 *mantissa)
{
 ...
 */
static inline u32 brcmf_twt_float_to_u32(u8 exponent, u16 mantissa)
{
...
brcmf_twt_stats_read(struct seq_file *seq, void *data)
{
	...
	for (i = 0; i < BRCMF_MAX_IFS; i++) {
		...
		list_for_each_entry(twt_sess, &ifp->twt_sess_list, list) {
			...
			if (twt_params->negotiation_type == BRCMF_TWT_CTRL_NEGO_TYPE_ITWT)
				seq_printf(seq, "\tiTWT Session, Flow ID: %u\n",
					   twt_params->flow_id);
			else if (twt_params->negotiation_type == BRCMF_TWT_CTRL_NEGO_TYPE_BTWT)
...
s32 brcmf_twt_cleanup_sessions(struct brcmf_if *ifp)
{
	...
	list_for_each_entry_safe(entry, next, &ifp->twt_sess_list, list) {
		...
		kfree(entry);
		brcmf_dbg(TWT,
			  "TWT: Deleted session with peer: %pM, flow ID: %d",
			  entry->peer_addr, entry->twt_params.flow_id);
	}
...
brcmf_itwt_lookup_session_by_flowid(struct brcmf_if *ifp, u8 flow_id)
{
	...
	list_for_each_entry(iter, &ifp->twt_sess_list, list) {
		if (iter->twt_params.negotiation_type !=
		    BRCMF_TWT_CTRL_NEGO_TYPE_ITWT)
			continue;
 ...
 */
static s32 brcmf_twt_update_session_state(struct brcmf_if *ifp,
					  struct brcmf_twt_session *twt_sess,
					  enum brcmf_twt_session_state state)
{
	...
	brcmf_dbg(TWT,
		  "TWT: updated session with peer: %pM, "
		  "flow ID: %d, state: %s",
		  twt_sess->peer_addr, twt_sess->twt_params.flow_id,
		  brcmf_twt_session_state_str[twt_sess->state]);
 ...
 * return: 0 on successful updation, value < 0 on failure.
 */
static s32 brcmf_twt_update_session(struct brcmf_if *ifp,
				    struct brcmf_twt_session *twt_sess,
				    const u8 *peer_addr,
				    enum brcmf_twt_session_state state,
				    struct brcmf_twt_params *twt_params)
{
	...
	memcpy(twt_sess->peer_addr, peer_addr, ETH_ALEN);
	twt_sess->state = state;
 ...
 */
static s32 brcmf_twt_del_session(struct brcmf_if *ifp,
				 struct brcmf_twt_session *twt_sess)
{
	s32 ret = 0;
	u8 flow_id;
	...
	memcpy(peer_addr, twt_sess->peer_addr, ETH_ALEN);
 ...
 */
static s32 brcmf_twt_add_session(struct brcmf_if *ifp, const u8 *peer_addr,
				 enum brcmf_twt_session_state state,
				 struct brcmf_twt_params *twt_params)
{
	...
	memcpy(new_twt_sess->peer_addr, peer_addr, ETH_ALEN);
	...
	brcmf_dbg(TWT,
		  "TWT: Added session with peer: %pM, "
		  "flow ID: %d, state: %s",
		  new_twt_sess->peer_addr, new_twt_sess->twt_params.flow_id,
 ...
 * return: 0 on success, value < 0 on failure.
 */
static s32 brcmf_twt_setup_event_handler(struct brcmf_if *ifp,
					 ...
					 void *data)
{
	...
	/* Setup Event */
	if (setup_desc->setup_cmd != BRCMF_TWT_SETUP_CMD_ACCEPT_TWT) {
	...
	/* Flowflags */
	twt_params.implicit =
		(setup_desc->flow_flags & BRCMF_TWT_FLOW_FLAG_IMPLICIT) ? 1 : 0;
	twt_params.flow_type =
		(setup_desc->flow_flags & BRCMF_TWT_FLOW_FLAG_UNANNOUNCED) ? 1 :
									     0;
	twt_params.trigger =
		(setup_desc->flow_flags & BRCMF_TWT_FLOW_FLAG_TRIGGER) ? 1 : 0;
	twt_params.requestor =
		(setup_desc->flow_flags & BRCMF_TWT_FLOW_FLAG_REQUEST) ? 1 : 0;
	twt_params.protection =
		(setup_desc->flow_flags & BRCMF_TWT_FLOW_FLAG_PROTECT) ? 1 : 0;
	...
	twt_params.min_twt = brcmf_twt_wake_dur_to_min_twt(
		le32_to_cpu(setup_desc->wake_dur), twt_params.min_twt_unit);
	...
	if (twt_sess)
		ret = brcmf_twt_update_session(
			ifp, twt_sess, e->addr,
			BRCMF_TWT_SESS_STATE_SETUP_COMPLETE, &twt_params);
	else
	...
	brcmf_dbg(TWT,
		  "TWT: Setup EVENT: Session Setup Complete\n"
		  "Setup command	: %u\n"
		  "Flow flags		: 0x %02x\n"
		  "Flow ID		: %u\n"
		  "Broadcast TWT ID	: %u\n"
		  ...
		  "Wake Interval	: %u uS\n"
		  "Negotiation type	: %u\n",
		  setup_desc->setup_cmd, setup_desc->flow_flags,
		  setup_desc->flow_id, setup_desc->bid, setup_desc->wake_time_h,
		  setup_desc->wake_time_l, setup_desc->wake_type,
		  setup_desc->wake_dur, setup_desc->wake_int,
 ...
 * return: 0 on success, value < 0 on failure.
 */
static s32 brcmf_twt_teardown_event_handler(struct brcmf_if *ifp,
					    const struct brcmf_event_msg *e,
					    void *data)
{
	...
	case BRCMF_TWT_CTRL_NEGO_TYPE_ITWT:
		...
		/* Lookup the session list for the received flow ID */
		twt_sess = brcmf_itwt_lookup_session_by_flowid(
			ifp, twt_params.flow_id);
		if (twt_sess) {
			ret = brcmf_twt_update_session_state(
				ifp, twt_sess,
				BRCMF_TWT_SESS_STATE_TEARDOWN_COMPLETE);
			if (ret) {
				brcmf_err(
					"TWT: Failed to update session state");
				goto exit;
			}
			...
			if (ret) {
				brcmf_err(
					"TWT: Failed to Delete session from list");
				goto exit;
			}
		} else {
			brcmf_dbg(TWT,
				  "TWT: session is not available to delete");
			ret = -EINVAL;
			goto exit;
		}
		break;
	...
	brcmf_dbg(TWT,
		  "TWT: Teardown EVENT: Session Teardown Complete\n"
		  "Flow ID		: %u\n"
		  "Broadcast TWT ID	: %u\n"
		  "Negotiation type	: %u\n"
		  "Teardown all TWT	: %u\n",
		  teardown_desc->flow_id, teardown_desc->bid,
		  teardown_desc->negotiation_type, teardown_desc->alltwt);
exit:
	return ret;
}
 ...
 * return: 0 on success, value < 0 on failure.
 */
s32 brcmf_notify_twt_event(struct brcmf_if *ifp,
			   const struct brcmf_event_msg *e, void *data)
{
	...
	case BRCMF_E_TWT_SETUP:
		...
		if (ret) {
			brcmf_err(
				"TWT: EVENT: Failed to handle TWT Setup event");
			goto exit;
		}
		break;
	case BRCMF_E_TWT_TEARDOWN:
		...
		if (ret) {
			brcmf_err(
				"TWT: EVENT: Failed to handle TWT Teardown event");
			goto exit;
		}
		break;
	default:
		brcmf_err("TWT: EVENT: Received event %d not handeled",
			  e->event_code);
		ret = -EOPNOTSUPP;
		goto exit;
	}
 ...
 * return: 0 on success, value < 0 on failure.
 */
static s32 brcmf_twt_setup_oper_handler(struct brcmf_if *ifp,
					struct brcmf_twt_params twt_params)
{
	...
	case BRCMF_TWT_CTRL_NEGO_TYPE_ITWT:
		...
		if ((twt_params.flow_id >= 0x0 && twt_params.flow_id <= 0x7)) {
			...
			if (twt_sess) {
				brcmf_err(
					"TWT: Setup REQ: Skipping, "
					"session with flow ID %d current state %s",
					twt_params.flow_id,
					brcmf_twt_session_state_str
						[twt_sess->state]);
				ret = -EINVAL;
				goto exit;
			}
	...
	val.sdesc.flow_flags |= ((twt_params.negotiation_type & 0x02) >> 1 ?
					 BRCMF_TWT_FLOW_FLAG_BROADCAST :
					 0);
	val.sdesc.flow_flags |=
		(twt_params.implicit ? BRCMF_TWT_FLOW_FLAG_IMPLICIT : 0);
	val.sdesc.flow_flags |=
		(twt_params.flow_type ? BRCMF_TWT_FLOW_FLAG_UNANNOUNCED : 0);
	val.sdesc.flow_flags |=
		(twt_params.trigger ? BRCMF_TWT_FLOW_FLAG_TRIGGER : 0);
	val.sdesc.flow_flags |= ((twt_params.negotiation_type & 0x01) ?
					 BRCMF_TWT_FLOW_FLAG_WAKE_TBTT_NEGO :
					 0);
	val.sdesc.flow_flags |=
		(twt_params.requestor ? BRCMF_TWT_FLOW_FLAG_REQUEST : 0);
	val.sdesc.flow_flags |=
		(twt_params.protection ? BRCMF_TWT_FLOW_FLAG_PROTECT : 0);

	if (twt_params.twt) {
		/* Target Wake Time parameter */
		val.sdesc.wake_time_h =
			cpu_to_le32((u32)(twt_params.twt >> 32));
		...
		val.sdesc.wake_time_h =
			cpu_to_le32((u32)(twt_params.twt_offset >> 32));
		val.sdesc.wake_time_l =
			cpu_to_le32((u32)(twt_params.twt_offset));
	...
	/* Wake Duration or Service Period */
	val.sdesc.wake_dur = cpu_to_le32(brcmf_twt_min_twt_to_wake_dur(
		twt_params.min_twt, twt_params.min_twt_unit));

	/* Wake Interval or Service Interval */
	val.sdesc.wake_int = cpu_to_le32(brcmf_twt_float_to_u32(
		twt_params.exponent, twt_params.mantissa));
	...
	if (twt_params.flow_id != 0xFF) {
		ret = brcmf_twt_add_session(
			ifp, vif->profile.bssid,
			BRCMF_TWT_SESS_STATE_SETUP_INPROGRESS, &twt_params);
		if (ret < 0) {
	...
	brcmf_dbg(TWT,
		  "TWT: Setup REQ: Session Setup In Progress\n"
		  "Setup command	: %u\n"
		  "Flow flags		: 0x %02x\n"
		  "Flow ID		: %u\n"
		  "Broadcast TWT ID	: %u\n"
		  ...
		  "Wake Interval	: %u uS\n"
		  "Negotiation type	: %u\n",
		  val.sdesc.setup_cmd, val.sdesc.flow_flags, val.sdesc.flow_id,
		  val.sdesc.bid, val.sdesc.wake_time_h, val.sdesc.wake_time_l,
		  val.sdesc.wake_type, val.sdesc.wake_dur, val.sdesc.wake_int,
 ...
 * return: 0 on success, value < 0 on failure.
 */
static s32 brcmf_twt_teardown_oper_handler(struct brcmf_if *ifp,
					   struct brcmf_twt_params twt_params)
{
		...
		if ((twt_params.flow_id >= 0x0 && twt_params.flow_id <= 0x7)) {
			...
			if ((twt_sess == NULL) ||
			    (twt_sess->state !=
			     BRCMF_TWT_SESS_STATE_SETUP_COMPLETE)) {
				brcmf_err(
					"TWT: Teardown REQ: flow ID: %d is not active",
					twt_params.flow_id);
				ret = -EINVAL;
				goto exit;
			}
	...
	brcmf_twt_update_session_state(
		ifp, twt_sess, BRCMF_TWT_SESS_STATE_TEARDOWN_INPROGRESS);
	brcmf_dbg(TWT,
		  "TWT: Teardown REQ: Session Teardown In Progress\n"
		  "Flow ID		: %u\n"
		  "Broadcast TWT ID	: %u\n"
		  "Negotiation type	: %u\n"
		  "Teardown all TWT	: %u\n",
 ...
 * return: 0 on success, value < 0 on failure.
 */
s32 brcmf_twt_oper(struct wiphy *wiphy, struct wireless_dev *wdev,
		   struct brcmf_twt_params twt_params)
{
	...
	if (!brcmf_feat_is_enabled(ifp, BRCMF_FEAT_TWT)) {
		brcmf_err(
			"TWT: REQ: Operation %d can't be handled, TWT not enabled",
			twt_params.twt_oper);
		ret = -EOPNOTSUPP;
		goto exit;
	}
	...
	if (wdev->iftype != NL80211_IFTYPE_STATION) {
		brcmf_err(
			"TWT: REQ: Operation %d can't be handled, vif is not STA",
			twt_params.twt_oper);
		ret = -EOPNOTSUPP;
		goto exit;
	}
	...
	if (!test_bit(BRCMF_VIF_STATUS_CONNECTED, &vif->sme_state)) {
		brcmf_err(
			"TWT: REQ: Operation %d can't be handled, vif not connected with WLAN peer",
			twt_params.twt_oper);
		ret = -ENOTCONN;
		goto exit;
	}
	...
	case BRCMF_TWT_CMD_SETUP:
		ret = brcmf_twt_setup_oper_handler(ifp, twt_params);
		break;
	case BRCMF_TWT_CMD_TEARDOWN:
