...
#define BRCMF_WLC_CNT_VER_6 6
#define BRCMF_WLC_CNT_VER_10 10
#define BRCMF_WLC_CNT_VER_30 30
				    ...
				    struct ethtool_drvinfo *info)
{
	...
	if (!drvr->cnt_ver) {
		...
		if (!iovar_out)
			return;
		ret = brcmf_fil_iovar_data_get(ifp, "counters", iovar_out,
					       WL_CNT_IOV_BUF);
		if (ret) {
...
static void brcmf_et_get_strings(struct net_device *net_dev, u32 sset,
				 u8 *strings)
{
...
static int brcmf_find_wlc_cntr_tlv(u8 *src, u16 *len)
{
	...
	while (cur_tlv < *len) {
		...
		memcpy(&data_len, (src + cur_tlv + XTLV_TYPE_SIZE),
		       sizeof(*len));
			       ...
			       struct ethtool_stats *et_stats, u64 *results_buf)
{
	...
	if (version >= BRCMF_WLC_CNT_VER_30) {
